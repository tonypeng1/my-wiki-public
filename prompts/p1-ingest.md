...
6. For each new file, do the following in order:
   ...
   b. Create wiki/summaries/{filename}.md following the 
      ...
      vocabularies in CLAUDE.md ("Provenance fields"). Three rules decide
      the hard cases:
      ...
      the row to the CLAUDE.md table AND to the sets in
      `scripts/check-provenance-fields.py` before using the slug — a slug
      that exists in neither is indistinguishable from a typo and fails
      /lint.
